...
def copy_password():
    if generate:
        gui.clipboard_clear()
        gui.clipboard_append(generate)
        result_label.config(text="Copied to Clipboard: " + generate)
    else:
        result_label.config(text="⚠️ No password to copy")
...
Button(gui, text="Copy Password", command=copy_password, bg="lightgreen", width=20).grid(row=4, column=0, padx=50, pady=10)
